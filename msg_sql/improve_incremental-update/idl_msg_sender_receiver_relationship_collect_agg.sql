...
ALTER TABLE idl_msg_sender_receiver_relationship_collect_agg DROP PARTITION (ds<="{p3}");
ALTER TABLE idl_msg_sender_receiver_relationship_collect_agg DROP PARTITION (ds="{p0}");
INSERT INTO idl_msg_sender_receiver_relationship_collect_agg PARTITION (ds="{p0}")
SELECT
...
FULL OUTER JOIN
    (SELECT *
    FROM idl_msg_sender_receiver_relationship_collect_tmp
    WHERE ds="{p0}"
    ) t2
